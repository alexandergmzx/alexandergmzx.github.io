...
| Homepage bio text                      | [`_pages/about.md`](_pages/about.md) — HTML body below front matter (one short paragraph)                                                                            |
| Homepage eyebrow, specialties, strip   | [`_layouts/about.liquid`](_layouts/about.liquid) — hardcoded lines; see [section 3](#3-about-page-homepage)                                                          |
| Vision & Venture page (`/vision/`)     | [`_pages/vision.md`](_pages/vision.md) — the longer personal piece; see [section 11](#11-vision--venture-page)                                                       |
...
| The walkthrough bundle itself          | `assets/swarm/` — **generated, never hand-edited**; see [section 5](#the-swarm-tab)                                                                                  |
| CV content (web page `/cv/`)           | [`_data/cv.yml`](_data/cv.yml)                                                                                                                                       |
| Downloadable CV PDF                    | export from the Google Doc and replace `assets/pdf/alexander_gomez_cv.pdf` (not generated by CI)                                                                     |
...
The homepage carries one short paragraph. The longer personal piece is `/vision/`
([section 11](#11-vision--venture-page)); the full record is the CV ([section 8](#8-cv-rendercv-format)).
...
title: about
permalink: /
subtitle: Embedded Systems · Robotics · Real-time Monitoring # not rendered by the custom layout
profile:
  align: right
...
announcements:
  enabled: true
  scrollable: false
  limit: 5 # how many "Recent updates" rows the homepage shows
---
```

The body is HTML, not Markdown, and keeps its two wrappers (styled in
[`_sass/_academic.scss`](_sass/_academic.scss)):

```html
<p class="intro-lead">I build software that has to answer to hardware.</p>

<div class="body-copy">
  <p>One paragraph, with links to <a href="/swarm/">SWARM</a>, <a href="/vision/">Vision</a> and the <a href="/cv/">CV</a>.</p>
</div>
```

### Copy that lives in the layout

[`_layouts/about.liquid`](_layouts/about.liquid) hardcodes:

- the eyebrow above the name (`<p class="eyebrow">Embedded systems · Robotics</p>`);
- the specialties line (`Real-time software / ROS 2 robotics / Embedded firmware`);
- the portrait caption (`Digital Systems & Robotics / Tec de Monterrey`);
- the "In practice" strip (`DXC Technology · Dextra Technologies · Amdocs`). No client names for the current employer.

### What the custom layout ignores

`subtitle`, `profile.more_info`, `social`, `selected_papers` and `latest_posts` are
stock al-folio keys; this layout does not read them. The homepage's meta description
is `description:` in [`_config.yml`](_config.yml), because `about.md` has none.

### Common edits

- **New profile photo:** drop the image in `assets/img/`, change `profile.image:`.
- **Fewer or more updates:** `announcements: limit:`.
- **Selected work rows:** [`_data/featured_projects.yml`](_data/featured_projects.yml).
...
4. **CV** (`_pages/cv.md`, `nav_order: 4`)
5. **More ▾** (`_pages/dropdown.md`, `nav_order: 7`) — contains Spectral (`/spectral/`), Swarm (`/swarm/`), Bookshelf, Vision & Venture, Repositories. Those pages are `nav: false`; the dropdown's `children:` list is what puts them in the navbar.
...
- `img:` is optional: without it the card on `/projects/` renders text-only
  (e.g. the parcel-sortation fleet). A project without `img` cannot go into
  `_data/featured_projects.yml`: the homepage row prints the image unconditionally.
...
`assets/swarm/index.html`, which opens it (`#/project`). The bundle is the walkthrough alone; every demo path,
an old `#/overview?demo=1` link included, opens it. Its step-5 figure ships in two captures, `swarm_walkthrough_step5.png` and
...
the generated files. The package contains one recording (`incident.json`) and two archives (`incident.pmtiles`, the street map,
and `context.pmtiles`, the city map), with a 15 MB budget enforced at build time. JavaScript chunks retain
...
`#/project?step=N` refresh, and that an old `#/overview?demo=1` link under `/assets/swarm/` opens the walkthrough.
...
The web CV at `/cv/` is rendered from this YAML by [`_layouts/cv.liquid`](_layouts/cv.liquid)
and the includes in [`_includes/cv/`](_includes/cv/). The downloadable PDF is **not**
generated from it (see below). Keep the two in step by hand.

### Top-level fields the layout prints

```yaml
cv:
  name: Jesus Alexander Gomez Gomez
  label: Embedded and Robotics Engineer # "Professional Title" row
  website: https://alexander-gomez.com # "Website" row
  summary: "One paragraph; Markdown links work." # "Professional Summary" card
  address:
    city: Mexico City # printed as "city, region"
    region: Mexico
  sections: ...
```

`location`, `image`, `address.countryCode` and `social_networks` are never printed.

### Sections

Experience always renders first. Every other section renders in YAML order, and
the layout picks its renderer by the **exact** section title:

| Title                                 | Fields each entry uses                                                                             |
| ------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `Experience`                          | `company`, `position` (bold), `location`, `start_date`, `end_date`, `summary`, `highlights`, `url` |
| `Education`                           | `institution`, `studyType` (bold), `area` (italic), `location`, dates, `url`, `highlights`         |
| `Projects` / `Open Source Projects`   | `name`, `url`, `summary`, `highlights` (dates are ignored: put the period in `summary`)            |
| `Skills` / `Interests`                | `name`, `keywords` (a YAML list)                                                                   |
| `Awards` / `Honors and Awards`        | `title`, `awarder`, `date`, `summary`                                                              |
| `Certificates` (not `Certifications`) | `name`, `issuer`, `date` (year shown)                                                              |
| `Languages`                           | `name`, `summary`                                                                                  |
| `Publications`, `References`          | see [`_includes/cv/`](_includes/cv/)                                                               |
| any other title                       | only `label` + `details`, or `bullet`                                                              |

Rules this CV follows:

- **An open-ended role needs `end_date: Present` written out.** A missing `end_date`
  prints `2026 -` with nothing after it.
- Dates are `YYYY-MM`; only the year is shown.
- `summary` and `highlights` go through Markdown, so `[SWARM](/swarm/)` links work.
  Quote any string containing `: ` or ` #`.
- One place per item: the RWTH summer school, the CentroGeo diploma and the UdG
  diploma live in Education, not Certificates.
- Numbers carry a status (measured, simulated, target); no em dashes; no client
  names under the current employer.

### Replacing the downloadable PDF

The PDF is exported from the CV Google Doc. Update the doc from `cv.yml`, export,
and overwrite `assets/pdf/alexander_gomez_cv.pdf`. Three links point at that path:
`cv_pdf:` in [`_pages/cv.md`](_pages/cv.md), `cv_pdf:` in
[`_data/socials.yml`](_data/socials.yml), and "Download CV" in
[`_layouts/about.liquid`](_layouts/about.liquid).

The RenderCV workflow ([`.github/workflows/render-cv.yml`](.github/workflows/render-cv.yml))
runs only by hand: `cv.yml` uses keys RenderCV rejects, so a push trigger failed on
every CV edit.
...
`assets/json/resume.json` (still the al-folio example). Don't switch unless you want
to rewrite the data file.
...
Standard `page` layout, reached from the More ▾ dropdown. It is the deeper,
personal piece; the homepage keeps one paragraph and the CV keeps the record.
Order of the body:

1. An opening paragraph, then four `##` sections: _Why embedded systems_,
   _Where SWARM came from_, _Research direction_, _The long horizon_.
2. The Tesla quote (a blockquote; attribution on its own `>` paragraph).
3. `## Maker journey`: a Markdown table, one row per year, `Now` last.
4. `## Values`: a raw HTML grid of 15 images from `assets/img/Values/`.
   Keep a blank line above and below the `<div>`.

The same claims rules apply as on the SWARM page: the radio link is measured, the
fourteen-node response is simulated, 50 m and four hours are targets, and
satellite collaboration is a direction, not a result.

Gotcha: `description: #text` is a YAML comment, so the page prints an empty
...
### A space and `#` start a YAML comment

```yaml
description: #Where I am headed # the value is empty: everything after " #" is a comment
description: "Where I am headed #1" # quote it to keep the #
```

### Open-ended CV roles

`end_date: Present` must be written out in `_data/cv.yml`; see [section 8](#8-cv-rendercv-format).
